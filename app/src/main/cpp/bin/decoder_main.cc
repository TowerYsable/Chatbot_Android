...
int main(int argc, char *argv[]) {
  ...
  auto model = std::make_shared<asr_cc::TorchAsrModel>();
  model->Read(FLAGS_model_path, FLAGS_num_threads);
  asr_cc::SymbolTable symbol_table(FLAGS_dict_path);
  asr_cc::DecodeOptions decode_config;
  decode_config.chunk_size = FLAGS_chunk_size;
  decode_config.num_left_chunks = FLAGS_num_left_chunks;
  asr_cc::FeaturePipelineConfig feature_config;
  feature_config.num_bins = FLAGS_num_bins;
  const int sample_rate = 16000;
  auto feature_pipeline =
      std::make_shared<asr_cc::FeaturePipeline>(feature_config);
  ...
  for (auto &wav : waves) {
    asr_cc::WavReader wav_reader(wav.second);
    ...
    asr_cc::TorchAsrDecoder decoder(feature_pipeline, model, symbol_table,
                                   decode_config);
